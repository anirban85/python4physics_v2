...
<main>
    ...
    <div class="content">
        <div class="pyodide-warning">
            <strong>Client-side execution:</strong> Python runs in your browser using <em>Pyodide</em>. Heavy or infinite loops may freeze your browser tab. If youhave heavy jobs, run locally.
        </div>
        ...
        <script>
            /***********************
             * Client-side Pyodide runner (Fixed + complete)
             ***********************/
            ...
            // Lazy-load pyodide and required packages (numpy, matplotlib, scipy)
            async function ensurePyodide() {
                ...
                try {
                    ...
                    // load core packages
                    await pyodide.loadPackage(['micropip']); // micropip is optional but often handy
                    // load numpy, matplotlib, scipy from pyodide packages
                    await pyodide.loadPackage('numpy');
                    await pyodide.loadPackage('matplotlib');
                    await pyodide.loadPackage('scipy');

                    // Inject bootstrap helpers: capture stdout/stderr, capture images, override plt.show/savefig
                    await pyodide.runPythonAsync(`
# bootstrap for pyodide runner
import sys, io, base64
from js import console

# captured images: list of base64-encoded PNG strings
...
def _pyodide_save_image_bytes(buf_bytes):
    try:
        ...
        console.warn('image encode failed:', str(e))

# Safe matplotlib setup and overrides
try:
    import matplotlib
    matplotlib.use('Agg')  # non-GUI backend (required in browser)
    import matplotlib.pyplot as plt
    from io import BytesIO
    
    # override plt.savefig to capture current figure as PNG bytes
    ...
    def _savefig_capture(*args, **kwargs):
        try:
            ...
            buf = BytesIO()
            # ensure PNG format
            fig.savefig(buf, format='png', **{k:v for k,v in kwargs.items() if k!='format'})
            buf.seek(0)
            _pyodide_save_image_bytes(buf.read())
        except Exception as e:
            console.warn('savefig capture failed:', str(e))
    plt.savefig = _savefig_capture

    # override plt.show() to capture current figure(s) as PNG bytes (user-friendly)
    def _show_capture(*args, **kwargs):
        try:
            ...
            console.warn('show capture failed:', str(e))
    plt.show = _show_capture

except Exception as e:
    console.warn('matplotlib bootstrap failed:', str(e))
`);

                    pyodideReady = true;
                } catch (err) {
                    console.error("Failed to load Pyodide or packages:", err);
                    alert("Failed to load Pyodide. See console for details.");
                    throw err;
                } finally {
            ...
            async function runCode(editorId, outputId) {
                ...
                // Clear previously captured images in Python env
                try {
                    await pyodide.runPythonAsync(`_pyodide_captured_images.clear()`);
                } catch(e) {
                    // ignore
                }

                // Build wrapper that captures stdout/stderr and returns outputs and images
                // Indent user code so try/except sits correctly
...
_images = globals().get('_pyodide_captured_images', [])
_out, _err, _images
`;

                showSpinner(true);

                // Set a client-side timeout: if code does not complete in X ms, reject
                const TIMEOUT_MS = 30000; // 30s browser-side timeout (adjustable)
                let timedOut = false;
                ...
                try {
                    ...
                    // result is a Python tuple-like PyProxy: (out, err, images)
                    // Convert to JS types
                    const out = result.get(0) ? result.get(0).toString() : "";
                    const err = result.get(1) ? result.get(1).toString() : "";
                    const images = result.get(2).toJs ? result.get(2).toJs() : [];
                    ...
                    // images is list of base64 strings (PNG)
                    ...
                    if ((!out || out.trim() === "") && (!err || err.trim() === "") && (!images || images.length === 0)) {
                ...
                document.querySelectorAll('.run-btn').forEach(btn => {
                    btn.addEventListener('click', async function(e){
                        ...
                        try {
                            ...
                            this.disabled = false;
                            this.textContent = prevText || 'Run Code';
                        }
                    });
                });
